...
**`optimize:clear` is never the last command.** It must always be followed by `optimize`, in the same session. With
the view cache empty, Blade recompiles the whole Filament view tree on every request: a single panel page fans out
into the page request plus one per lazy widget plus the notification poll, all compiling the same templates into
`storage/framework/views` at once. Each compile writes a temporary file and renames it over the destination, so the
concurrent requests race. On Linux the loser's rename silently wins and the cost is only the wasted compile; on a
Windows machine the rename is refused outright (`Access is denied (code: 5)`) and the request 500s. The release and
rollback runbooks (sections 5 and 7) already pair the two commands — keep them paired in any ad-hoc session too.

`view:cache` is overridden by `App\Console\Commands\CacheViewsCommand`, which compiles each template under both the
path spelling the framework uses (`SplFileInfo::getRealPath()`) and the one `FileViewFinder` produces at request time.
On Linux the two are byte-identical, so the command compiles exactly what the framework's does and production output
is unchanged; on Windows they differ and only the override leaves a cache the request can actually read.

A few templates are generated by Blade at runtime (inline component strings) and cannot be precompiled by any
warm-up command. They are written on the first request after a cache clear, so load one page before opening the site
to users; from the second request on, nothing compiles.
