...
/**
 * Lead reassignment through the panel (D-4): the record and bulk `assign`
 * actions on the lead list and page are offered only to assign-holders, reach
 * only records and users inside the actor's scope, and every change is
 * audited as `lead.assigned` and notified to the new owner.
 *
 ...
 */
final class LeadAssignmentTest extends TestCase
{
    ...
    public function a_rep_is_never_offered_the_assign_actions(): void
    {
        ...
        $this->assertFalse($rep->can('assign', $lead), 'D-4: reps never reassign');

        Livewire::actingAs($rep)
            ->test(ListLeads::class)
            ->set('activeTab', 'all')
            ->assertTableActionHidden('assign', $lead);
        ...
        Livewire::actingAs($rep)
            ->test(ListLeads::class)
            ->set('activeTab', 'all')
            ->callTableBulkAction('assign', [$lead], data: ['owner_id' => null]);
    ...
    #[Test]
    public function a_manager_reassigns_a_team_lead_from_the_list_with_an_audit_entry_and_a_notification(): void
    {
        Notification::fake();
        ...
        $lead = Lead::factory()->create(['owner_id' => $from->getKey()]);

        Livewire::actingAs($manager)
            ->test(ListLeads::class)
            ->set('activeTab', 'all')
            ->assertTableActionVisible('assign', $lead)
            ->callTableAction('assign', $lead, data: ['owner_id' => $to->getKey()])
            ->assertHasNoTableActionErrors()
            ->assertNotified(__('assignment.notifications.assigned', ['name' => $to->name]));

        $this->assertSame($to->getKey(), $lead->refresh()->owner_id);

        $log = ActivityLog::query()->where('description', ActivityLogEvent::LeadAssigned->value)->latest('id')->firstOrFail();
        $this->assertSame($lead->getKey(), (int) $log->subject_id);
    ...
    #[Test]
    public function a_manager_cannot_hand_a_lead_to_someone_outside_the_team(): void
    {
        $team = $this->makeTeam();
        $manager = $this->salesManager($team);
        ...
        Livewire::actingAs($manager)
            ->test(ListLeads::class)
            ->set('activeTab', 'all')
            ->callTableAction('assign', $lead, data: ['owner_id' => $outsider->getKey()])
        ...
        app(RecordAssignmentService::class)->assign($lead, $outsider, $manager);
    }
    ...
    public function a_manager_reassigns_from_the_lead_page(): void
    {
        ...
        $this->assertSame($manager->getKey(), $lead->refresh()->owner_id);
    }

    #[Test]
    public function bulk_assignment_skips_the_leads_outside_the_actor_scope(): void
    {
        Notification::fake();
        ...
        $this->assertSame(
            [$inTeam->getKey()],
            ActivityLog::query()->where('description', ActivityLogEvent::LeadAssigned->value)->pluck('subject_id')->map(static fn (mixed $id): int => (int) $id)->all(),
        );
